 ...
 * @param k     first derivative of curvature (1/m2)
 */
pub fn odr_spiral(s: f32, k: f32) -> (f32, f32, f32) {
    let a = k.abs().sqrt().recip() * PI.sqrt();
    let t_ang = s.powi(2) * k * 0.5;

    let (y, x) = fresnel(s / a);

    (x * a, y * k.signum() * a, t_ang)
}
